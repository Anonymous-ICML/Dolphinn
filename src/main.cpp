...
int main()
{
	if(MAX_PNTS_TO_SEARCH > N)
	{
  	...
  	read_points_IDX_format<T>(pointset, N, D, "path/to/Points/File"); //replace the file path with your own
	...
	//print_2D_vector<T>(pointset, N, D);
  	...
  	read_points_IDX_format<T>(query, Q, D, "path/to/Points/File"); //replace the file path with your own
